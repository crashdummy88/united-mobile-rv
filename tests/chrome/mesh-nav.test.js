 ...
 * Prefer Text tel:+16166065277 — never Text Now or Text / Call.
...
test('mesh helper lists Main/Forum/Software/Status/Portal/Shop/Docs as absolute hosts', () => {
  ...
  for (const html of [header, footer, mobile]) {
    ...
    assert.doesNotMatch(html, /Text Now/);
...
test('shop + forum templates include mesh-chrome (or static mesh + Square)', () => {
  ...
  for (const file of forumSsr) {
    ...
    assert.doesNotMatch(html, /Text Now/, file);
  }
  ...
  assert.doesNotMatch(forum, /Text Now/);
  ...
  const features = src('forum/features.js');
  assert.match(features, /Prefer Text \(616\) 606-5277/);
  assert.doesNotMatch(features, /Text Now/);
  assert.doesNotMatch(features, /Text Us/);
});
...
test('site.js rewrites chrome Book to Square and Text Now / Text / Call to Prefer Text', () => {
  const js = src('js/site.js');
  assert.match(js, /https:\/\/united-mobile-rv-llc\.square\.site\//);
  assert.match(js, /PREFER_TEXT_HREF = 'tel:\+16166065277'/);
  assert.match(js, /Prefer Text \(616\) 606-5277/);
  assert.match(js, /\^\(Book\|Book Now\|Book a Service\)\$/);
  assert.match(js, /umrt-platform-bar/);
  assert.match(js, /option value="Text" selected>Prefer Text/);
  assert.doesNotMatch(js, /BOOK_PUBLIC = 'https:\/\/book\.unitedmobilerv\.com\//);
  assert.doesNotMatch(js, /TEXT_NOW_HREF = 'sms:/);
  assert.doesNotMatch(js, /pages\.dev/);
});

test('mothership home + platform-bar Book chrome hits Square + Prefer Text', () => {
