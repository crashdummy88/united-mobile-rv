...
test('MESH_LINKS is Home · Services · Guides · Shop · Book · Forum · Software · Docs', () => {
  ...
  for (const html of [header, footer]) {
    ...
    assert.match(html, /https:\/\/unitedmobilerv\.com\/services\//);
  ...
  assert.match(mobile, /href="tel:\+16166065277" aria-label="Call \(616\) 606-5277">Call</);
  assert.match(mobile, /tel:\+16166065277/);
  assert.match(mobile, /class="mobile-text-now"[^>]+sms:\+16166065277/);
  assert.match(mobile, />Text Now</);
  assert.doesNotMatch(mobile, /Text Now \(616\) 606-5277/);
  assert.match(mobile, /href="https:\/\/forum\.unitedmobilerv\.com\/">Join the Free Forum</);
  assert.doesNotMatch(mobile, /btn-gold|btn-ghost|class="btn/);
...
test('shop + forum templates include mesh-chrome (or static mesh + Square)', () => {
  ...
  assert.match(forum, /href="https:\/\/unitedmobilerv\.com\/services\/"[^>]*>Services</);
...
test('site.js stamps Call + gold Text Now + Square Book on every nav/mobile bar', () => {
  ...
  const navCta = js.match(/var navCtaHtml = [\s\S]*?;/)[0];
  const mobileBar = js.match(/var mobileBarHtml = [\s\S]*?;/)[0];
  assert.doesNotMatch(navCta, /Join the Free Forum/);
  assert.match(navCta, /btn-gold/);
  assert.match(js, /var FORUM_JOIN_HREF = 'https:\/\/forum\.unitedmobilerv\.com\/'/);
  assert.match(js, /var FORUM_JOIN_LABEL = 'Join the Free Forum'/);
  assert.match(js, /var MOBILE_CALL_LABEL = 'Call'/);
  assert.match(mobileBar, /FORUM_JOIN_LABEL/);
  assert.match(mobileBar, /class="mobile-text-now"/);
  assert.match(mobileBar, /aria-label="' \+ CALL_LABEL \+ '">/);
  assert.match(mobileBar, /href="' \+ FORUM_JOIN_HREF \+ '">' \+ FORUM_JOIN_LABEL/);
  assert.doesNotMatch(mobileBar, /btn-gold|btn-ghost/);
