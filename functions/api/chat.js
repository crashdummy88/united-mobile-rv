...
const FALLBACK =
  "Live chat AI is offline right now. Text Now (616) 606-5277 — or Book at /book-service/. A human will follow up.";
...
const SCRIPTED_ANSWERS = [
  ...
  },
  {
    ...
    reply:
      "We don't run discounts or coupon codes through chat. If you've got a pricing question specific to your job, I'll flag it for our team to follow up on directly — go ahead and share your name, phone, and issue and we'll reach out.",
  },
  {
...
export async function onRequestPost(context) {
  ...
  if (rl.limited) {
    return json({ error: 'rate_limited', reply: FALLBACK, retry_after: rl.retryAfter }, 429);
  }
  ...
  if (!tsResult.ok) {
    return json({ error: 'captcha_failed', reply: FALLBACK }, 403);
  }
  ...
  const leadNote = lead
    ...
    : 'No structured lead yet — ask for Name, Phone, Email, Location, Rig, issue, Text if booking.';

  if (!env.AI) {
    return json({
      reply: FALLBACK,
      mode: 'fallback',
      hint: 'Workers AI binding missing — add AI binding on Cloudflare Pages project',
    });
  }
...
async function callWorkersAI(AI, model, leadNote, messages) {
  const chatMessages = [
  ...
  const text = (result && (result.response || result.text || result.result)) 
    ? String(result.response || result.text || result.result).trim()
    : '';
  if (!text && result && typeof result === 'object') {
    // some models return { description } / nested
    const nested = result.result?.response || result.result?.text;
