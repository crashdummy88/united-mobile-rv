...
export async function onRequestPost(context) {
  ...
  if (isSeedOrPinSpamId(threadId)) return json({ success: false, error: 'not_found' }, 404);
  const thread = await env.DB.prepare('SELECT id, author_id, hidden FROM threads WHERE id = ? AND hidden = 0').bind(threadId).first();
