...
export async function onRequestGet(context) {
  ...
  const thread = await env.DB.prepare(
    `SELECT t.id, t.title, t.body, t.category, t.author_id, t.created_at, t.updated_at, t.pinned, t.solved, t.solved_at, t.accepted_post_id, t.image_keys, u.display_name AS author, u.avatar_url AS author_avatar
     FROM threads t JOIN users u ON u.id = t.author_id WHERE t.id = ? AND t.hidden = 0`
  ...
  const { results: posts } = await env.DB.prepare(
    `SELECT p.id, p.body, p.created_at, p.image_keys, u.display_name AS author, u.avatar_url AS author_avatar
     FROM posts p JOIN users u ON u.id = p.author_id WHERE p.thread_id = ? AND p.hidden = 0 ORDER BY p.created_at ASC`
  ).bind(params.id).all();
...
export async function onRequestPost(context) {
  ...
  if (!thread) return json({ success: false, error: 'not_found' }, 404);
  const recent = await env.DB.prepare(`SELECT created_at FROM posts WHERE author_id = ? ORDER BY created_at DESC LIMIT 1`).bind(session.uid).first();
  if (recent) {
  ...
  const id = randomId();
  await env.DB.prepare(`INSERT INTO posts (id, thread_id, author_id, body, hidden, ai_flagged, ai_reason, image_keys) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, params.id, session.uid, text, hidden, mod.flagged ? 1 : 0, mod.reason, imageKeysJson).run();
  if (!hidden) {
    ...
    context.waitUntil(notifyForumActivity(env, { subject: `New forum reply: ${threadRow ? threadRow.title : params.id}`, message: `New reply from ${session.name || 'a member'}:\n\n${text}\n\nhttps://united-mobile-rv.pages.dev/forum/` }));
  }
  if (mod.flagged) await env.DB.prepare(`INSERT INTO moderation_log (id, post_id, thread_id, author_id, action, reason) VALUES (?, ?, ?, ?, ?, ?)`).bind(randomId(), id, params.id, session.uid, hidden ? 'auto_hide' : 'flagged', mod.reason).run();
