...
from commerce.pricing import price, calculate_markup_price  # noqa: E402
...
def build_update_sql(product_id: str, priced, apply_live_pricing: bool = False) -> str:
    assignments = [
    ...
    # --apply-live-pricing (2026-09-14): the one explicit, opt-in exception
    # to "never writes retail_price" -- see pricing.py's calculate_markup_price
    # docstring for the full reasoning. Only fires when the caller passes
    # the flag AND we have a real cost to calculate from; competitor_price
    # is not wired up here (no confirmed, compliant source for one yet --
    # see chat/README) so this is markup+MAP only until that changes.
    if apply_live_pricing and priced.cost is not None:
        markup = calculate_markup_price(
            product_id=product_id, cost=priced.cost, map_price=priced.map_price,
        )
        if markup.calculated_price is not None:
            assignments.append(f"retail_price = {sql_str(markup.calculated_price)}")
            assignments.append(
                f"price_source = {sql_str(f'auto-calculated ({markup.rule_applied}), ' + priced.last_checked)}"
            )
...
def run(supplier_key: str, products_json: Path | None, apply_live_pricing: bool = False) -> Path:
    ...
    for record in records:
        ...
        log_line = (f"  [sync] {record.product_id} | cost={priced.cost} map={priced.map_price} "
                    f"stock={priced.stock_status} -> {priced.price_status}")
        if apply_live_pricing and priced.cost is not None:
            markup = calculate_markup_price(product_id=record.product_id, cost=priced.cost, map_price=priced.map_price)
            log_line += f" | retail_price -> {markup.calculated_price} ({markup.rule_applied})"
        print(log_line)
        statements.append(build_update_sql(record.product_id, priced, apply_live_pricing=apply_live_pricing))

    GENERATED_DIR.mkdir(parents=True, exist_ok=True)
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    suffix = "_livepricing" if apply_live_pricing else ""
    out_path = GENERATED_DIR / f"{supplier_key}_sync{suffix}_{ts}.sql"
    live_pricing_warning = (
        "-- *** --apply-live-pricing WAS USED *** -- this file also OVERWRITES\n"
        "-- retail_price (auto-calculated: cost + markup, clamped to MAP) --\n"
        "-- normally sync.py never touches retail_price. Read every line.\n\n"
        if apply_live_pricing else ""
    )
    header = (
        f"-- Generated by commerce/sync.py -- supplier: {supplier_key}\n"
        f"-- {len(statements)} product(s) updated, {skipped} skipped (no D1 match).\n"
        f"{live_pricing_warning}"
...
def main() -> int:
    ...
    parser.add_argument(
        "--apply-live-pricing", action="store_true",
        help="Also calculate and write retail_price (cost + markup, clamped to MAP). "
             "Deliberate reversal of the default 'never touch retail_price' behavior -- "
             "still just writes a reviewable SQL file unless combined with --apply-local/--apply-remote.",
    )
    args = parser.parse_args()

    sql_path = run(args.supplier, args.products_json, apply_live_pricing=args.apply_live_pricing)
    if sql_path is None:
        return 0
